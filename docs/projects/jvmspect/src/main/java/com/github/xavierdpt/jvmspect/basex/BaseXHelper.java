...
public class BaseXHelper {
    public static BaseXSession startServer() throws IOException {
        ...
        File tmpDir = new File(JVMSpectConstants.LOCAL_DIR, "tmp");
    ...
    public static void runQueryFile(String port, File tmpDir, String databaseName, File queryFile, Consumer<FileReader> readerConsumer) throws IOException {
        File outputFile = new File(tmpDir, "basex.out");
        new BaseXClient("-p" + port, "-Uadmin", "-Padmin",
                "-o" + outputFile.getAbsolutePath(),
                "-i" + databaseName,
